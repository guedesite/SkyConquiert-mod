...
public class BoxUltimeTileEntity extends TileEntity implements IInventory
{
    ...
    private World w;
    private static final BoxUltimeItem[] Box = main.getUltimeBox();
    private boolean isRemote = true, isOpen = false, isScrolling = false;
    public BoxUltimeContainer container;
    public static Thread o, i;
    public BoxUltimeTileEntity(EntityPlayer player, boolean isRemote, World w) {
		this.player = player;
	...
	public void giveWin()
	{
		if(this.contents[3] != null)
		{
			...
			item2.setStackDisplayName(this.contents[3].getDisplayName().replace(" "+C.d_purple+"["+C.l_purple+"Box"+C.d_purple+"]"+C.l_purple, ""));
	...
	public void openInventory() {
		...
		if(!this.isRemote && this.player != null)
		{
			i = new Thread() {
				@Override
				public void run() {
	...
	public boolean isItemValidForSlot(int p_94041_1_, ItemStack p_94041_2_)
	 {
    	...
    	if(p_94041_1_ == 0)
    	{
    		if(p_94041_2_.getItem() == ItemMod.Ultime_Key)
    		{
    			return true;
    		}
    		else
    	...
    	{
    		if(p_94041_2_.getDisplayName().contains(C.d_purple+"["+C.l_purple+"Box"+C.d_purple+"]"+C.l_purple))
    		{
    			return true;
    		}
    		else
