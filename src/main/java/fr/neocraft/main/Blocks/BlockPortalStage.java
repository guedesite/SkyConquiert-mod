...
public class BlockPortalStage extends BlockBreakable
{
   
    public static fr.neocraft.main.bdd bdd = main.getbdd();
    private int stageid = 0;
    
    public BlockPortalStage(String txt, int dimid)
    {
        super(neoreference.MOD_ID + ":Portal_" + txt, Material.portal, false);
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
    {
    	...
    	{
    		if(player.getCurrentEquippedItem().getItem() == ItemMod.Portal_Transformer)
    		{
    			if(!world.isRemote)
    			{
	    			try {
	    				int nb = Integer.parseInt(player.getCurrentEquippedItem().getDisplayName());
	    				if(world.getBlockMetadata(x, y, z) != nb)
	    				{
	    					world.setBlockMetadataWithNotify(x, y, z, nb, 2);
	    					player.addChatMessage(new ChatComponentTranslation("New MetaData for: "+x+":"+y+":"+z));
	    				
	    					return true;
	    				}
	    			} catch(NumberFormatException e)
	    			{
	    				player.addChatMessage(new ChatComponentTranslation("neo.DENY_ERROR"));
	    			}
    			}
    			else
    			{
    ...
    {
    	BlockPortalStage.Size size = new BlockPortalStage.Size(p_150000_1_, p_150000_2_, p_150000_3_, p_150000_4_, 1);
    	BlockPortalStage.Size size1 = new BlockPortalStage.Size(p_150000_1_, p_150000_2_, p_150000_3_, p_150000_4_, 2);
     ...
     */
       ...
       {
       	if(entity instanceof EntityPlayer & !w.isRemote){
       		if (entity.ridingEntity == null && entity.riddenByEntity == null)
               {
       			if(timeForPortal <= timeUntilPortal || timeUntilPortal == 0)
       			{
					...
					EntityPlayer player = (EntityPlayer) entity;
       					...
       					teleport.player(this.stageid, player , 8, 102, -7, 2, false);
