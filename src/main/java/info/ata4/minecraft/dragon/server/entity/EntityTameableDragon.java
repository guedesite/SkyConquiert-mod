...
public class EntityTameableDragon extends EntityFlyingTameable {
    ...
    public static double BASE_SPEED_AIR = 0.3;
    public static double BASE_DAMAGE = 3;
    public static double BASE_HEALTH = 20;
    public static float BASE_DAMAGE_FIRE = 0.4F;
    public static double BASE_SPEED_FIRE = 0.05D;
    public static boolean canfireball = true;
    public static boolean mustfireball = false;
    
    public static final float BASE_WIDTH = 4;
    public static final float BASE_HEIGHT = 3f;
    public static final int HOME_RADIUS = 30;
    public static final Item FAVORITE_FOOD = Items.fish;
    private int RidingStat = 0;
    // data value IDs
    ...
    public EntityTameableDragon(World world, PlayerStats stat) {
        super(world);
        if(stat.lvlGuiDrag[0] < 3)
        {
        	canfireball = false;
        }else {
        }
        BASE_HEALTH = 20 + 20 * stat.lvlGuiDrag[1];
        BASE_SPEED_GROUND = 0.3 + 0.1 * stat.lvlGuiDrag[2];
        BASE_SPEED_AIR = 0.3 + 0.5 * stat.lvlGuiDrag[3];
        BASE_DAMAGE_FIRE = 0.4F + 0.2F * stat.lvlGuiDrag[4];
        BASE_SPEED_FIRE = 0.05D + 0.05 * stat.lvlGuiDrag[5];
    ...
    public void BouBoule() {
    	Vec3 looking = this.getOwner().getLookVec();
      	 if (looking != null) {

      		 double motionX = looking.xCoord;
      		 double motionY = looking.yCoord;
      		 double motionZ = looking.zCoord;

  	         double d8 = 4.0D;
  	         Vec3 vec3 = this.getLookVec();
  	         double posX = this.posX + vec3.xCoord * d8,posY = this.posY + (double)(this.height / 2.0F) + 0.5D,posZ = this.posZ + vec3.zCoord * d8; 
  	         double rx2,ry2,rz2, rx, ry, rz;
  	         for(int i = 0;  i<9; i++)
  	         {
  	        	 rx = ((double)this.worldObj.rand.nextInt(6))/10-0.5;
	   	         ry = ((double) this.worldObj.rand.nextInt(6))/10-0.5;
	   	         rz = ((double)this.worldObj.rand.nextInt(6))/10-0.5;
	   	         rx2 = ((double)this.worldObj.rand.nextInt(2))/10-0.1;
	   	         ry2 = ((double) this.worldObj.rand.nextInt(2))/10-0.1;
	   	         rz2 = ((double)this.worldObj.rand.nextInt(2))/10-0.1;
	   	         DragFire f = new DragFire(this.worldObj,motionX+rx2, motionY+ry2, motionZ+rz2, this.getOwner().getCommandSenderName());
	   	         f.setPosition(posX+rx, posY+ry, posZ+rz);
	   	         this.worldObj.spawnEntityInWorld(f);
      	 ...
      	 {
      		 System.err.println("look vec null drag");
      	 }
    }
        ...
        if (isClient()) {
            ...
            if (isTamed()) {
                ...
                if (owner != null) {
                	...
                	if(this.RidingStat == 0)
                	{
                		if(!(owner.ridingEntity == this))
                		{
                		...
                		{
                			this.setRotationYawHead(owner.getRotationYawHead()); 
                		}
 
                	}
                	else if(this.RidingStat > 0)
                	{
                		this.RidingStat--;
                	}
                }
    ...
    public void setRidingStat(int i)
    {
    	this.RidingStat = i;
    }
    @Override
    ...
    public void spawnFireBall()
    {
    	 ...
    	 if (looking != null) {
    		...
    		/* double motionX = looking.xCoord;
    		 double motionY = looking.yCoord;
    		 double motionZ = looking.zCoord;
    	 double accelerationX = motionX * BASE_SPEED_FIRE;
    		 double accelerationY = motionY * BASE_SPEED_FIRE;
    		 double accelerationZ = motionZ * BASE_SPEED_FIRE; 
    		 double accelerationX = motionX;
    		 double accelerationY = motionY;
    		 double accelerationZ = motionZ;
	         double d8 = 4.0D;
	         Vec3 vec3 = this.getLookVec();
	         double posX = this.posX + vec3.xCoord * d8;
	         double posY = this.posY + (double)(this.height / 2.0F) + 0.5D;
	         double posZ = this.posZ + vec3.zCoord * d8; */
	         this.worldObj.spawnParticle("lava", this.posX, this.posY+2, this.posZ,0D, 0D, 0D);
    	 }
    }
    /**
    ...
    protected void func_145780_a(int x, int y, int z, Block block) {
        if (isEgg() || inWater) {
            ...
            worldObj.playSoundAtEntity(this, neoreference.MOD_ID + ":mob.enderdragon.step", 0.5f, 1);
        }
    }
