...
public class CommandDragon extends CommandBase {
    ...
    public String getCommandUsage(ICommandSender sender) {
        ...
        return String.format("/dragon <stage <%s>>|<breed <%s>> | <sql <%s %s [player]>> [global]", stages, breeds, stages, breeds);
    }
        ...
        if (command.equals("stage")) {
            ...
            if (sender instanceof EntityPlayerMP) {
        	 ...
        	 if (params.length < 4) {
	        	 ...
	        	 String breedName = params[1].toLowerCase();
	        	 DragonBreed breed = DragonBreedRegistry.getInstance().getBreedByName(breedName);
	        	 String stage = params[2].toLowerCase();
	        	 String player = params[3].toLowerCase();
	        	 EntityPlayerMP p = (EntityPlayerMP) sender;
	        	 if(bdd.Exist("SELECT * FROM "+bdd.getStringDrag()+" WHERE pseudo='" + player +"'"))
	        	 {
	        		 p.addChatMessage(new ChatComponentTranslation("dragon.neocraft.change"));
	        		 bdd.update("UPDATE `neo-dragon` SET `breed`='" + breed.toString() + "',`stage`='" + stage.toUpperCase() + "' WHERE pseudo='" + player +"'");
	        	 }
	        	 else
	        	 {
	        		 p.addChatMessage(new ChatComponentTranslation("dragon.neocraft.change"));
	        		 bdd.execute("INSERT INTO `neo-dragon`(`breed`, `stage`, `pseudo`) VALUES ('" + breed.toString() + "','" + stage + "','" + player + "')");
	        	 }
